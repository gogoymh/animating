...
from gan import style_mixer, generator, discriminator

print("="*100)
##############################################################################################################################
parser = argparse.ArgumentParser()
parser.add_argument("--exp_name", type=str, default="Drawing", help="Experiment index")
parser.add_argument("--dataset", type=str, default="Gan_test", help="Dataset")
...
for iteration in range(opt.iteration):
    ...
    joint, rand_joint, image = train_loader.__iter__().next()
    ...
    with torch.no_grad():
        style_1 = styler(joint)
        style_2 = styler(rand_joint)
        
        num = np.random.choice(64,1)[0]
        where = np.random.choice(64, num, replace=False)
        
        style_2[:, where] = style_1[:, where]
        
        fake = G(style_2)
        
    real_pred = D(image, style_1)
    fake_pred = D(fake, style_2)
    ...
    if g_regularize:
        ...
        style_1 = styler(joint[:path_batch_size])
        style_2 = styler(rand_joint[:path_batch_size])
